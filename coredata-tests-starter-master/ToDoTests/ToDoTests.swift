...
class ToDoTests: XCTestCase {
    ...
    override func setUp() {
        ...
        // 1. create mock items for the tests
        createMockItems()
        
        // 2. instantiate  the manager and inject the mockPersistantcontainer as the needed dependency.
    ...
    // Check that created item is not nil
    func test_create_todo() {
        ...
        //When adding an item
        let item = manager.insertTodoItem( name: name, completed: completionStatus )
        manager.save()
    ...
    func test_fetch_all_todo() {
        ...
        //When adding an item
        let _ = manager.insertTodoItem(name: name, completed: completion)
        let _ = manager.insertTodoItem(name: name2, completed: completion2)
        manager.save()
        
        //Assert the returned item is the same amount you inserted
        XCTAssertEqual(itemsTotalCount(), 5)
    }
    ...
    func test_remove_todo() {
        ...
        // Given the name & completion status
        let arrayItems = manager.fetchAllItems()
        let idFirstItem = arrayItems[0].objectID
        
        // When removing an item
        manager.removeItem( objectID: idFirstItem )
        manager.save()
        
        // Assert the returned items in the array has the right amount
    ...
    func createMockItems() {
        ...
        //Create a few items, ignore the unused warning you'll get. This items are also called stubs. This is where we get all the items wihtin all our test functions so we can test on them!! :o
        let _ = insertTodoItem(name: "Sarin", completed: false)
        let _ = insertTodoItem(name: "walk home", completed: false)
        let _ = insertTodoItem(name: "workout in the morning", completed: true)
